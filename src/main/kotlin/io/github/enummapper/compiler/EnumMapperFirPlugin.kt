...
public class EnumMapperCompilerPluginRegistrar : CompilerPluginRegistrar() {
    override val pluginId: String = "io.github.enummapper"
    override val supportsK2: Boolean = true

    override fun ExtensionStorage.registerExtensions(configuration: CompilerConfiguration) {
        FirExtensionRegistrarAdapter.registerExtension(EnumMapperFirExtensionRegistrar())
        IrGenerationExtension.registerExtension(ObjectMappingIrExtension())
    }
}

private class EnumMapperFirExtensionRegistrar : FirExtensionRegistrar() {
    override fun ExtensionRegistrarContext.configurePlugin() {
        +::EnumMapperAdditionalCheckers
        registerDiagnosticContainers(EnumMapperFirDiagnostics)
    }
}

private class EnumMapperAdditionalCheckers(session: FirSession) :
...
private object EnumMappingCallChecker : FirFunctionCallChecker(MppCheckerKind.Common) {
    private val mapToId = CallableId(FqName("io.github.enummapper"), Name.identifier("mapTo"))
    private val javaMapId = CallableId(
        FqName("io.github.enummapper"),
        FqName("EnumMapper"),
        Name.identifier("map"),
    )
    ...
    override fun check(expression: FirFunctionCall) {
        ...
        val sourceType = when (callableId) {
            mapToId -> expression.extensionReceiver?.resolvedType
            javaMapId -> expression.argumentList.arguments.firstOrNull()?.resolvedType
            else -> return
        } ?: return
        ...
        reporter.reportOn(expression.source, EnumMapperFirDiagnostics.MISSING_ENUM_CONSTANTS, message)
    }
    ...
    private fun validateObjectMapping(expression: FirFunctionCall) {
        ...
        if (problems.isNotEmpty()) {
            reporter.reportOn(
                expression.source,
                EnumMapperFirDiagnostics.MISSING_ENUM_CONSTANTS,
...
private object EnumMapperFirDiagnostics : KtDiagnosticsContainer() {
    override fun getRendererFactory(): BaseDiagnosticRendererFactory = EnumMapperFirMessages
...
private object EnumMapperFirMessages : BaseDiagnosticRendererFactory() {
    override val MAP: KtDiagnosticFactoryToRendererMap by
        KtDiagnosticFactoryToRendererMap("ENUM_MAPPER") { map ->
            map.put(
                EnumMapperFirDiagnostics.MISSING_ENUM_CONSTANTS,
                "{0}",
